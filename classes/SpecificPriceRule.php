                ...
                foreach ($conditionGroup as $idCondition => $condition) {
                    if ($condition['type'] == 'attribute') {
                        ...
                        if ($idAttr >= 0) {
                            ...
                            $idAttr = -$idAttr;
                            $query->where('NOT EXISTS(SELECT 1 FROM `'._DB_PREFIX_.'product_attribute` pa2 INNER JOIN `'._DB_PREFIX_.'product_attribute_combination` pac2 ON pa2.id_product_attribute = pac2.id_product_attribute WHERE pac2.id_attribute = '.$idAttr.' AND pa2.id_product = p.id_product)');
                        }
